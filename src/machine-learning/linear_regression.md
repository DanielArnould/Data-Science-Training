...
\\[
\text{MSE} = \underset{\text{Calculate average}}{\underbrace{\frac{1}{N}}} \overset{\text{Range over all N samples of training data}}{\overbrace{\sum_{i=1}^{n}}}(\underset{\text{actual output for the ith data point}}{\underbrace{y_i}} - \underset{\text{predicted output for the ith data point}}{\underbrace{\hat{y_i}}})^2
\\]

Before we go any further, let's just recontextualise this in terms of our advertising example. We'll first only consider 1 variable (TV advertising) just to keep things simple. Here \\(i\\) refers to the \\(ith\\) company in our table

\\[
\text{Sales}_i = w\text{TV}_i + b
\\]
...
\\[
\mathcal{L}(w, b) = \frac{1}{N}\sum_{i=1}^{n}(y_i - (w\text{TV}_i + b))^2
\\]
...
So how do we find the derivative of \\(\mathcal{L}\\) with respect to \\(w\\) and \\(b\\)? Since we need to consider \\(w\\) and \\(b\\) independently, we need to find their partial derivatives. We can find their partial derivatives by noticing that \\((y_i - (w\text{TV}\_i + b))^2\\) is really 2 nested functions: the inner function \\(y_i - (w\text{TV}\_i + b)\\) and the outer function $x^2$. This means, we can just use the [Chain rule](https://tutorial.math.lamar.edu/classes/calci/chainrule.aspx) that you learnt in high school!
...
_Step 1_: Because the derivative of a sum is the sum of the derivatives, we can focus on finding the derivative of each \\((y_i - (w\text{TV}\_i + b))^2\\). So define \\(\mathcal{L}\_i\\):

\\[
\mathcal{L}_i(w, b) = (y_i - (w\text{TV}_i + b))^2
\\]

_Step 2:_ Differentiate with respect to $w$:

Using the chain rule, let \\(u = y_i - (w \text{TV}\_i + b)\\). Then \\(\mathcal{L}\_i (w, b) = u^2\\).
...
\\[
\frac{\partial \mathcal{L_i}}{\partial u} = 2u = 2(y_i - (w \text{TV}_i + b))
\\]

Then, compute \\(\frac{\partial u}{\partial w}\\):

\\[
\frac{\partial u}{\partial w} = -\text{TV}_i
\\]

Combine these results and simplify:

\\[
\frac{\partial \mathcal{L_i}}{\partial w} = 2(y_i - (w \text{TV}_i + b)) \times -\text{TV}_i
\\]

\\[
\frac{\partial \mathcal{L_i}}{\partial w} = -2\text{TV}_i(y_i - (w \text{TV}_i + b))
\\]

_Step 3:_ Differentiate with respect to $b$:

Using the chain rule again, let \\(u = y_i - (w \text{TV}\_i + b)\\). Then \\(\mathcal{L}\_i (w, b) = u^2\\).
...
\\[
\frac{\partial \mathcal{L_i}}{\partial b} = -2(y_i - (w \text{TV}_i + b))
\\]
...
\frac{1}{N} \sum_{i=1}^{n} -2TV_i(y_i - (w TV_i + b)) \newline
\frac{1}{N} \sum_{i=1}^{n} -2(y_i - (w TV_i + b))
\end{bmatrix}
\\]

So, after calculating the gradient, we can then update our parameters \\(w\*j\\) and \\(b\*j\\) during iteration \\(j\\) to \\( w\_{j+1} \\)and \\( b\_{j+1} \\) for the next iteration \\( j+1 \\)

\\[
w_{j + 1} = w_j - \gamma \frac{\partial \mathcal{L}}{w_j}
\\]

\\[
b_{j + 1} = w_j - \gamma \frac{\partial \mathcal{L}}{b_j}
\\]
...
Fortunately, the math doesn't actually get that much more complicated even as we add more variables (features). We just have to keep track of more partial derivatives, but the way we calculate those partial derivatives is the same! So, using all the columns in our table, our functions would just change accordingly:

\\[
\text{Sales} = w_1 \text{Social Media} + w_2\text{TV} + w_3\text{Billboards} + b
\\]

\\[
\mathcal{L}(w_1, w_2, w_3, b) = \frac{1}{N}\sum_{i=1}^{n}(y_i - (w_1 \text{Social Media}_i + w_2\text{TV}_i + w_3\text{Billboards}_i + b))^2
\\]

\\[
\mathcal{L}' (w_1, w_2, w_3, b) = \begin{bmatrix}
\frac{1}{N} \sum_{i=1}^{n} -2Social Media_i(y_i - (w_1Social Media_i + w_2TV_i + w_3Billboards_i + b)) \newline
\frac{1}{N} \sum_{i=1}^{n} -2TV_i(y_i - (w_1 Social Media_i + w_2TV_i + w_3Billboards_i + b)) \newline
\frac{1}{N} \sum_{i=1}^{n} -2Billboards_i(y_i - (w_1Social Media_i + w_2TV_i + w_3Billboards_i + b)) \newline
\frac{1}{N} \sum_{i=1}^{n} -2(y_i - (w_1Social Media_i + w_2TV_i + w_3Billboards_i + b))
\end{bmatrix}
\\]
...
\mathcal{L}' = \frac{-2}{3}
\begin{bmatrix}
\text{Social Media}_1 & \text{Social Media}_2 & \text{Social Media}_3 \newline
\text{TV}_1 & \text{TV}_2 & \text{TV}_3 \newline
\text{Billboards}_1 & \text{Billboards}_2 & \text{Billboards}_3 \newline
1 & 1 & 1 \newline
\end{bmatrix}
\begin{bmatrix}
y_1 - \hat{y_1} \newline
